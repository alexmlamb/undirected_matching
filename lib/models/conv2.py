...
the input. Latent variables z are derived by x-z uwing injected noise. In
addition, noise is added to z-x through concatenation. Finally, there is an
option to use multiple discriminator scores.
...
def init_gparams(p, n_levels=3, dim_z=128, dim_h=128, dim_c=3, dim_x=32,
    ...
    p = param_init_fflayer(
        options={}, params=p, prefix='n_z_ff', nin=dim_z,
    ...
    p = param_init_fflayer(
        ...
        nin=dim_h, nout=dim_z, ortho=False,
        batch_norm=False)
...
def init_dparams(p, n_levels=3, dim_z=128, dim_h=128, dim_hd=512, dim_c=3,
    ...
    for level in xrange(n_levels):
        ...
        if level == 0:
            dim_in = dim_z + dim_h * dim_x * dim_y // scale // 2
...
def z_to_x(p, z, n_levels=3, dim_h=128, dim_x=32, dim_y=32,
    ...
    z = T.concatenate([z, srng.normal(std=noise_std,
                                      size=z.shape, dtype=z.dtype)], axis=1)
    d['z'] = z
    ...
    for level in xrange(n_levels):
        ...
        if level == n_levels - 1:
            #activ = 'lambda x: x'
            #activ = 'lambda x: T.nnet.sigmoid(x)'
...
def x_to_z(p, x, n_levels=3, dim_z=128, dim_h=128, dim_c=3, dim_x=32, dim_y=32,
           normalize_z=False, return_tensors=False, noise_std=1., **kwargs):
    '''x to z transition.
    ...
    h_e = fflayer(tparams=p, state_below=epsilon, options={}, prefix='n_z_ff',
                  activ='lambda x: tensor.nnet.relu(x, alpha=0.02)')
    ...
    logger.debug('Forming layer with name {}'.format('x_z_final1/2'))
    h = fflayer(
        tparams=p, state_below=h, options={}, prefix='x_z_final1',
        activ='lambda x: tensor.nnet.relu(x, alpha=0.02)')
    z = fflayer(
