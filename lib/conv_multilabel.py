...
def train(train_fn, gen_fn,
    ...
    for epoch in xrange(num_epochs):
        ...
        lab = [[ATTRIBUTE_NAMES[c] for c in np.where(l_ == 1)[0].tolist()]
               for l_ in lab]
        #lab = [l_[:(l_.tolist().index(1) if 1 in l_ else len(l_))]
        #       for l_ in lab]
        #lab = [[''.join([CHAR_MAP[c] for c in l_])] for l_ in lab]
        plot_images(
            X=x_gen, file_path=path.join(
                image_dir, 'gen_epoch_{}'.format(epoch)),
            captions=lab)
        ...
        #       for l_ in lab]
        #lab = [[''.join([CHAR_MAP[c] for c in l_])] for l_ in lab]
        plot_images(X=R_NONLINEARITY(x_in_[:64]),
                    file_path=path.join(image_dir, 'gt'),
                    captions=lab)

        #cond gen
        y_x_chain = inpaint_fn_x(label_[:64], z_im[:64])
